...
// Using Lucide Icons for consistent, modern SVG icons
...
class IconManager {
    constructor() {
        this.icons = {
            ...
            'shield-check': '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path><path d="M9 12l2 2 4-4"></path></svg>',
            'shield-alert': '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>',
            ...
            // Business & Legal
            'scale': '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 16l3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1zM2 16l3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1z"></path><path d="M7 21h10M12 3v18M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"></path></svg>',
            'building': '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"></rect><path d="M9 22v-4h6v4M8 6h.01M12 6h.01M16 6h.01M8 10h.01M12 10h.01M16 10h.01M8 14h.01M12 14h.01M16 14h.01"></path></svg>',
    ...
    init() {
        // Replace all Lucide icon tags after DOM is loaded
        this.replaceAllIcons();
        
        // Watch for DOM changes to replace dynamically added icons
        this.observeDOM();
    }
    ...
    replaceAllIcons() {
        ...
        lucideElements.forEach(element => {
            ...
            this.replaceIcon(element, iconName);
        });
    }
                    ...
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === 1) { // Element node
                            if (node.hasAttribute && node.hasAttribute('data-lucide')) {
                                ...
                                this.replaceIcon(node, iconName);
                            }
                            // Check children
    ...
    // Helper method to create icon elements
    createElement(name, size = 24, className = '') {
        const span = document.createElement('span');
        span.className = 'icon-wrapper ' + className;
        span.innerHTML = this.getIcon(name, size, className);
        return span;
    }
}

// Initialize the icon manager when DOM is ready
